...
cat << EOF | sudo gpg --dearmor --output /etc/apt/keyrings/wakemeops.gpg
...
echo "Types: deb
Architectures: amd64
Signed-By: /etc/apt/keyrings/wakemeops.gpg
URIs: http://deb.wakemeops.com/wakemeops/
Suites: stable
Components: dev" | sudo tee /etc/apt/sources.list.d/wakemeops.sources
sudo apt update && sudo apt install --assume-yes glab
