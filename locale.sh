...
sudo sed --in-place '$d' /etc/locale.gen
sudo sed --in-place --expression "s|^de_BE\.UTF-8|# de_BE.UTF-8|" /etc/locale.gen
sudo sed --in-place --expression "s|^nl_BE\.UTF-8|# nl_BE.UTF-8|" /etc/locale.gen
sudo sed --in-place --expression "s|^# en_GB\.UTF-8|en_GB.UTF-8|" /etc/locale.gen
sudo sed --in-place --expression "s|^# en_US\.UTF-8|en_US.UTF-8|" /etc/locale.gen
...
sudo sed --in-place --expression "/LANGUAGE=.*/a LC_MESSAGES=C.UTF-8" /etc/default/locale
