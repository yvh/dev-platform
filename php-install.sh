...
sudo apt install --assume-yes php${VERSION}-{apcu,ast,bz2,cli,curl,fpm,gd,imagick,intl,ldap,mbstring,mysql,opcache,soap,solr,ssh2,readline,redis,xml,xsl,xdebug,zip}

for SYSTEM in fpm cli; do
...
sudo sed --in-place "s|listen.owner = www-data|listen.owner = yvh|" /etc/php/${VERSION}/fpm/pool.d/www.conf
sudo sed --in-place "s|listen.group = www-data|listen.group = yvh|" /etc/php/${VERSION}/fpm/pool.d/www.conf
sudo sed --in-place "s|user = www-data|user = yvh|" /etc/php/${VERSION}/fpm/pool.d/www.conf
sudo sed --in-place "s|group = www-data|group = yvh|" /etc/php/${VERSION}/fpm/pool.d/www.conf

#sudo sed --in-place --expression "/;env\[TEMP\]*/a ;env[https_proxy] = http://127.0.0.1:3128" /etc/php/${VERSION}/fpm/pool.d/www.conf
#sudo sed --in-place --expression "/;env\[TEMP\]*/a ;env[http_proxy] = http://127.0.0.1:3128" /etc/php/${VERSION}/fpm/pool.d/www.conf
